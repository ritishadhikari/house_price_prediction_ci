...
def main(args):
    ...
    model=get_model_instance(
        model_name=model_cfg['best_model'],
    ...
    with mlflow.start_run(run_name="final_training"):
        ...
        mae=float(mean_absolute_error(y_true=y_test,y_pred=y_pred))
        r2=float(r2_score(y_true=y_test,y_pred=y_pred))

        mlflow.log_params(params=model_cfg['parameters'])
        mlflow.log_metrics(metrics={'mae':mae,'r2':r2})

        # Log and Register Model
        save_model_name="tuned_model"
        mlflow.sklearn.log_model(model,save_model_name)
        model_uri=f"runs:/{mlflow.active_run().info.run_id}/{save_model_name}"
        ...
        client.set_registered_model_tag(model_name,"features","All features in the dataset except the target variable")
        ...
        logger.info(f"Final MAE: {mae:.2f}, R-squared: {r2:.4f}")
